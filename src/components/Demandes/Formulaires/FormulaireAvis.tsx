...
interface FormulaireAvisProps {
  onClose: () => void; // Pour revenir à la liste des demandes ou au tableau de bord
...
export default function FormulaireAvis({ onClose, onDemandeSubmitted }: FormulaireAvisProps) {
  const [page, setPage] = useState(1); // État pour la page actuelle du formulaire
  ...
  const validatePage = (pageNumber: number): boolean => {
    ...
    switch (pageNumber) {
      ...
      case 2:
        ...
        break;
      case 3:
        if (!formData.texteJuridique.trim()) {
          newErrors.texteJuridique = 'Ce champ est obligatoire';
          isValid = false;
        }
        break;
      case 4:
          ...
          {Array.from({ length: totalPages }, (_, i) => (
            <div
              ...
              className={`w-2 h-2 rounded-full ${
                i + 1 === page ? 'bg-[#FFA500]' : 'bg-gray-300'
              }`}
            />
          ))}
        </div>
          ...
          {options.map((option) => (
            <label key={option.value} className="inline-flex items-center">
              <input
                ...
                onChange={(e) => {
                  const values = formData[id] ? formData[id].split(',') : [];
                  if (e.target.checked) {
          ...
          <div className="space-y-6">
            ...
            {renderCheckboxGroup(
              ...
              [
                ...
                { value: 'clients_potentiels', label: 'Clients (actuels ou potentiels)' },
  ...
  const handleSaveDraft = () => {
    try {
      localStorage.setItem('formulaireAvisDraft', JSON.stringify(formData));
  ...
  const loadDraft = () => {
    try {
      const savedDraft = localStorage.getItem('formulaireAvisDraft');
      if (savedDraft) {
